...
let form = useForm('post', '/api/applications', {
    id: '',
    name: '',
    version: '',
    description: '',
...
function openCreateForm() {
    ...
    form = useForm('post', '/api/applications', {
        id: '',
        name: '',
        version: '',
        description: '',
...
async function openEditForm(row: any) {
    form = useForm('put', `/api/applications/${row.id}`, {
        id: row.id,
        name: row.name,
        version: row.version,
    ...
    selectedHost.value = await loadHostById(row.host_id) || null;
    showForm.value = true;
}
async function loadHostById(id: number) {
    if (!id) return null;
    const res = await fetch(`/api/application-hosts/${id}`);
    const data = await res.json(); // data harus { label, value }
    return data;
}
async function loadData(params: any) {
    ...
    try {
        ...
        const res = await fetch(`/api/applications?skip=${dtParams.value.first}&take=${dtParams.value.rows}&sortField=${sortField}&sortOrder=${sortOrder}&search=${encodeURIComponent(search.value)}`);
        ...
        <div class="flex h-full flex-col gap-4 p-4 overflow-x-auto">
            <div class="space-y-4">
                <div class="between flex items-center justify-between">
                    <HeadingSmall title="Applications" description="Manage your Applications" />
                    ...
                    <form @submit.prevent="submit">
                        <div class="flex flex-col gap-3">
                            ...
                            <div class="flex flex-col gap-1">
                                ...
                                <Message v-if="form.invalid('status')" severity="error" variant="simple" size="small"
                                    :closable="false">
                                    {{ form.errors.status }}
                                </Message>
                            </div>
                            ...
                            <div class="flex flex-col gap-1">
                                <Select :options="typeOptions" optionLabel="label" optionValue="value"
                                    v-model="form.type" placeholder="Select Type" @change="form.validate('type')" />
                                <Message v-if="form.invalid('type')" severity="error" variant="simple" size="small"
                                    :closable="false">
                                    {{ form.errors.type }}
                                </Message>
                            </div>
                            <div class="flex flex-col gap-1">
                                <AutoComplete v-model="selectedHost" :suggestions="hostSuggestions" optionLabel="label"
                                    optionValue="value" field="label" :forceSelection="true" :loading="loadingHosts" cl
                                    placeholder="Search or select host" @complete="searchHosts" dropdown />
